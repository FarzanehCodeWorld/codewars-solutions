...
class vendingMachine{
  constructor(items,initialMoney){
    this.items = items
    this.bank = toCents(initialMoney)
}
vend(selection,cash) {
  ...
  it(!item) return `Invalid selection!: Money in vending machine = ${toDollars(this.bank).toFixed(2)}`
  const price = toCents (tiem.price)
  if(price > cents) return 'Not enough money!'
  if(items.quality < 1) return `${item.name}:out of stock!`
