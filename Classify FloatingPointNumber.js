// The IEEE 754 double-precision encoding scheme
...
function getNumberType(number) {
    let num  = numToIEEE_754(number),
        ex   = num.substring(1, 12),
        mts  = num.substring(12),
        res = num.charAt(0) == "0" ? "POSITIVE_" : "NEGATIVE_"

    if (isNaN(number))     { return (mts.charAt(0) == "0" && /[1]+/.test(mts)) ? res + "SIGNALING_NAN" : res + "QUIET_NAN" } 
    if (!isFinite(number)) { return res + "INFINITY" }  
    if (number == 0)       { return res + "ZERO" }
  
    return (/^[0]+$/.test(ex) && /[1]+/.test(mts)) ? res + "DENORMALIZED" : res + "NORMALIZED"
}
...
function numToIEEE_754(num) {
    ...
    let str = ""
    for (let i = 7; i >= 0; i--) {
        str += uArr[i].toString(2).padStart(8, "0")
    }
    return str
}
