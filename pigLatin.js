...
// For example, the word "spaghetti" becomes "aghettispay" because the first two letters ("sp") are consonants, so they are moved to the end of the string and "ay" is appende

//Solution:
function pigLatingGame(string) {
  string = string.toLowerCase();
  if(string.search(/[^a-z]/) !== -1) return 'null';
  let pos = string.search(/[aeuio]/);
  //no vowels inside string
  if(pos == -1) return string+'ay'
  //first letter starts with vowel
