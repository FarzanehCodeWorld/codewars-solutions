...
function GuessWho(character) {
  ...
  this.character = this.characteristic.find(([name]) => name === character);
  this.tally = 0;
}
...
GuessWho.prototype.guess = function(guess) {
  ...
  if (guess === this.character[0]) {
    return ['Correct! in ' + this.tally + ' turns'];
  }

  const isGuessInNames = this.characteristic.some(([name]) => name === guess);

  const nameFilter = ([name]) => name !== guess;
  const charFilter = ([_, chars]) =>
    this.character[1].includes(guess)
      ? chars.includes(guess)
      : !chars.includes(guess);

  this.characteristic = this.characteristic.filter(
    isGuessInNames ? nameFilter : charFilter
  );
  return this.characteristic.map(([name]) => name);
};
