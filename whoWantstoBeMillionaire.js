...
function getTotalCashPrize(prizeFund, correctAnswers, playerActions) {
  const getPrize = (count) => prizeFund.reduce((s, n, i) => (i < count) ? s + n : s, 0);

  const calcPrize = (num, val) => {
    if (val === 'X') return getPrize(Math.floor((num) / 5) * 5);

    if (val === 'W') return getPrize(num);

    if (['A', 'B', 'C', 'D'].includes(val) && val === correctAnswers[num]) return getPrize(num + 1);

    return 0;
  };

  const actions = playerActions.map(a => a.replace(/[123]+/g, ''))
  const lifeLines = playerActions.join('').match(/\d{1}/g);

  return [
    calcPrize(actions.length - 1, actions[actions.length - 1]),
