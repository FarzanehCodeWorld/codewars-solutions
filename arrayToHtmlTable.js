...
const tr = str => `<tr>${str}</tr>`;
const th = str => `<th>${str}</th>`;

const thead = arr => {
	let result = arr.map(x => th(x)).join('');
	return `<thead>${tr(result)}</thead>`;	
}

const tbody = arr => {
	let result = arr.map(sub => tr(sub.map(x => td(x)).join(''))).join('');
	return `<tbody>${result}</tbody>`
};

const toTable = (data, headers = false, index = false) => {
	arr = data.slice(); // protect the source array
	arr = arr.map(sub => sub.map(x => x === null ? '' : x));

	result = '';
	if (headers) result += thead(arr.shift());
	if (index) {
		result = result.replace('<thead><tr>', '<thead><tr><th></th>');
		arr = arr.map((sub, i) => [i + 1, ...sub]);
	}
	result += tbody(arr);
